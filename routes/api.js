...
apiRouter.post('/vote', (req, res) => {
  const { voter, author, permlink } = req.body;
...
apiRouter.post('/comment', (req, res) => {
  const { parentAuthor, parentPermlink, author, permlink, title, body, jsonMetadata } = req.body;
  steem.broadcast.comment(
    req.wif,
    parentAuthor,
    parentPermlink,
...
apiRouter.post('/deleteComment', (req, res) => {
...
apiRouter.post('/reblog', (req, res) => {
  const { permlink, account, author } = req.body;
  const requiredAuths = [];
  const requiredPostingAuths = [req.username];
  const id = 'follow';
  const json = JSON.stringify(['reblog', { account, author, permlink }]);
...
apiRouter.post('/unfollow', (req, res) => {
...
apiRouter.post('/customJson', (req, res) => {
  const { id, json, requiredPostingAuths, requiredAuths } = req.body;
  steem.broadcast.customJson(
    req.wif,
    requiredAuths,
    requiredPostingAuths,
...
apiRouter.post('/claimRewardBalance', (req, res) => {
  const { account, rewardSteem, rewardSbd, rewardVests } = req.body;
  if (req.username !== account) {
    sendResponse({ err: 'account name must match with steemconnect account name' }, res);
  } else {
    steem.broadcast.claimRewardBalance(
